      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-8">
          {quickButtons.map((button) => (
            <Link
              key={button.title}
              href={button.href}
              className="group flex items-center justify-center gap-4 rounded-xl border-2 border-[#FF0000] bg-white px-5 py-3 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:bg-[#FF0000] hover:shadow-lg"
            >
              <Image
                src={button.icon}
                alt={button.alt}
              ...
              />
              <span className="text-lg font-semibold text-[#FF0000] transition-colors duration-300 group-hover:text-white sm:text-xl">
                {button.title}
              </span>
            </Link>
          ))}
        ...
        <div className="grid grid-cols-1 gap-8 text-sm sm:grid-cols-2 lg:grid-cols-5 lg:gap-7">
          ...
          <div className="lg:col-span-1">
            ...
            <Link
              href="/contact"
              className="mt-5 inline-flex rounded-xl border border-[#FF0000] bg-white px-4 py-2 text-lg font-semibold text-[#FF0000] transition-all duration-300 hover:bg-[#FF0000] hover:text-white hover:shadow-md"
            >
              Contact us
            </Link>
          </div>
            ...
            <div className="flex flex-col gap-2">
              {importantLinks.map((item) => (
                <Link
                  key={item.title}
                  href={item.href}
                  className="leading-5 transition-all duration-300 hover:translate-x-1 hover:text-[#FF0000]"
                >
                  {item.title}
                </Link>
              ))}
            ...
            <div className="flex flex-col gap-2">
              {serviceLinks.map((item) => (
                <Link
                  key={item.title}
                  href={item.href}
                  className="transition-all duration-300 hover:translate-x-1 hover:text-[#FF0000]"
                >
                  {item.title}
                </Link>
              ))}
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex flex-col items-center justify-center rounded-lg border border-[#FF0000] bg-white p-3 text-center shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        ...
        <div className="flex flex-col items-center justify-center gap-2 pt-5 text-center">
          ...
          <Link
            ...
            className="pb-2 text-sm transition-colors duration-300 hover:text-[#FF0000]"
          >
